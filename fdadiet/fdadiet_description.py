...
class fdadietDescriptionPage(webapp.RequestHandler):
    def get(self):
        ...
        html = html + template.render(templatepath + '02hh_uberintroblock_wmodellinks.html', {'model':'fdadiet','page':'description'})
        html = html + template.render(templatepath + '03hh_ubertext_links_left.html', {})                       
        html = html + template.render(templatepath + '04ubertext_start.html', {
                'model_page':'#', 
                'model_attributes':'FDA Dietary Exposure Model Overview', 
                'text_paragraph':xx})
...
app = webapp.WSGIApplication([('/.*', fdadietDescriptionPage)], debug=True)
