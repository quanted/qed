...
class genericAlgorithmPage(webapp.RequestHandler):
    def get(self):
        text_file1 = open('gompertz/gompertz_algorithm.txt','r')
        x = text_file1.read()         
        templatepath = os.path.dirname(__file__) + '/../templates/'
        html = template.render(templatepath + '01pop_uberheader.html', {'title'})
        html = html + template.render(templatepath + '02pop_uberintroblock_wmodellinks.html', {'model':'gompertz','page':'algorithm'})
        html = html + template.render(templatepath + '03pop_ubertext_links_left.html', {})                       
        html = html + template.render(templatepath + '04uberalgorithm_start.html', {
                'model':'gompertz', 
                'model_attributes':'Gompertz Model Algorithms', 
                'text_paragraph':x})
