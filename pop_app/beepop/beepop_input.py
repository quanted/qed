...
class beepopInputPage(webapp.RequestHandler):
    def get(self):
        templatepath = os.path.dirname(__file__) + '/../templates/'
        html = template.render(templatepath + '01pop_uberheader.html', {'title':'Ubertool'})
        html = html + template.render(templatepath + 'beepop-jQuery.html', {})
        html = html + template.render(templatepath + '02pop_uberintroblock_wmodellinks.html', {'model':'beepop','page':'input'})
...
app = webapp.WSGIApplication([('/.*', beepopInputPage)], debug=True)
