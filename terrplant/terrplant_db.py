...
class TerrPlantInp(forms.Form):
    ...
    incorporation = forms.ChoiceField(required=True, choices=SELECT_INCORPORATION, label=mark_safe('Incorporation (If incorporation depth is <u>&lt</u> 1 in, choose 1. If > 1 in, choose #. If <span style="padding-left:20px"></span><u>&gt</u> 6 in, choose 6. For Aerial applications, choose 1.)'))    
    application_rate = forms.FloatField(required=True,label='Application rate (lbs ai/A)',initial=4)    
    drift_fraction = forms.ChoiceField(required=True, choices=SELECT_DRIFT, label=mark_safe('Drift Fraction (Liquid application form by ground: 0.01. Liquid application by <span style="padding-left:20px"></span>aerial, airblast or spray chemigation: 0.05. Granular application form: 0.)'),initial=0.01)
    runoff_fraction = forms.ChoiceField(required=True, choices=SELECT_RUN, label=mark_safe('Runoff Fraction (For solubility < 10 ppm, choose 0.01. For solubility 10-100 < <span style="padding-left:20px"></span>ppm, choose 0.02. For solubility > 100 ppm, choose 0.05)'),initial=0.05)
