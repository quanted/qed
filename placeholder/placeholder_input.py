...
class placeholderInputPage(webapp.RequestHandler):
    def get(self):
        text_file = open('placeholder/placeholder_description.txt','r')
        x = text_file.read()
        templatepath = os.path.dirname(__file__) + '/../templates/'
        html = template.render(templatepath + '01hh_uberheader.html', {'title':'Ubertool'})
        html = html + template.render(templatepath + '02hh_uberintroblock_wmodellinks.html', {'model':'placeholder','page':'input'})
...
app = webapp.WSGIApplication([('/.*', placeholderInputPage)], debug=True)
