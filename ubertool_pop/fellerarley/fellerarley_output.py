...
def fellerarleygrow(N_o,T,rho,beta, Ite):
    index_set = range(T+1)
...
class fellerarleyOutputPage(webapp.RequestHandler):
    def post(self):        
        ...
        x=fellerarleygrow(N_o,T,rho,beta, Ite)[0]
        x_mu=fellerarleygrow(N_o,T,rho,beta, Ite)[1]
        
        templatepath = os.path.dirname(__file__) + '/../templates/'
        html = template.render(templatepath + '01pop_uberheader.html', {'title':'Ubertool'})
        html = html + template.render(templatepath + '02pop_uberintroblock_wmodellinks.html', {'model':'fellerarley','page':'output'})
...
app = webapp.WSGIApplication([('/.*', fellerarleyOutputPage)], debug=True)
