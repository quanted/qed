...
def generate_sam_input_file(args, sam_input_file_path):
    ...
    try:
        myfile.write("%s \n" % ' '.join(map(str, inputs[
            'sim_day_index_list'])))  # Map the list items to strings and join them together with " " (space); this print( list of strings without quotes around each list item)
    except Exception as e:
        print(str(e))
        myfile.write(' ---ERROR--- \n')
    try:
        myfile.write("%s \n" % ' '.join(map(str, inputs[
            'sim_day_date_list'])))  # Map the list items to strings and join them together with " " (space); this print( list of strings without quotes around each list item)
    except Exception as e:
        print(str(e))
        myfile.write(' ---ERROR--- \n')
    try:
