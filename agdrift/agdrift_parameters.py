...
#    extending_settings = forms.ChoiceField(required=True,label='Optional settings', choices=Extended_settings_CHOICES, initial='Make a selection')
    ...
    calculation_input = forms.ChoiceField(required=True, label = 'Toolbox Input Type', choices=Calculation_input_CHOICES, initial ='Make a selection')
    distance = forms.FloatField(required=True,label=mark_safe('Distance to water body or terrestrial point from edge of field (ft)'))
