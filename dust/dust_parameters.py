...
class DustInp(forms.Form):
    ...
    application_rate = forms.FloatField(required=True,label='Maximum Single Application Rate',initial=15.73)
    frac_pest_assumed_at_surface = forms.DecimalField(required=True,label='Fraction of Pesticide Assumed at Surface',initial=.3)
    dislodgeable_foliar_residue = forms.FloatField(required=True,label='Dislodgeable Foliar Residue',initial=5)
    ...
    low_bird_acute_oral_ld50 = forms.FloatField(required=True,label=mark_safe('Lowest Bird Acute Oral LD<sub>50</sub> <br>&asymp; Amphibian Dermal LD<sub>50</sub>'),initial=4640)
    tested_bird_body_weight = forms.FloatField(required=True,label='Tested Bird Body Weight',initial=1580)
    ...
    mamm_acute_derm_ld50 = forms.FloatField(required=True,label=mark_safe('Mammal Acute Dermal LD<sub>50</sub>'),initial=2000,)
    #aviandermaltype = forms.ChoiceField(required=True, choices=AvianDermalType_CHOICES, initial='With DTI', label='Avian Dermal type')
    mam_acute_oral_ld50 = forms.FloatField(required=True,label=mark_safe('Mammal Acute Oral LD<sub>50</sub>'),initial=3920)
    tested_mamm_body_weight = forms.FloatField(required=True,label='Tested Mammal Body Weight',initial=350)
