...
class TerrPlantBatchRunner():
    ...
    def runTerrPlantModel(self,config_properties,results_dict):
        ...
        if A and I and R:
            results_dict['EEC-dry'] = terrplant.rundry(A,I,R)
            results_dict['EEC-semi-aquatic'] = terrplant.runsemi(A, I, R)
            D = None
            if 'spray_drift' in config_properties:
                D = config_properties['spray_drift']
            if D:
                results_dict['EEC-spray-drift'] = terrplant.spray(A,D)
        if results_dict['EEC-dry'] and results_dict['EEC-spray-drift']:
            results_dict['EEC-total-dry'] = terrplant.totaldry(results_dict['EEC-dry'],results_dict['EEC-spray-drift'])
        if results_dict['EEC-semi-aquatic'] and results_dict['EEC-spray-drift']:
            results_dict['EEC-total-semi-aquatic'] = terrplant.totalsemi(results_dict['EEC-semi-aquatic'],results_dict['EEC-spray-drift'])
        nms = None
        ...
        if results_dict['EEC-total-dry'] and nms:
            results_dict['nmsRQdry'] = terrplant.nmsRQdry(results_dict['EEC-total-dry'],nms)
        if results_dict['EEC-total-semi-aquatic'] and nms:    
            results_dict['nmsRQsemi'] = terrplant.nmsRQsemi(results_dict['EEC-total-semi-aquatic'],nms)
            results_dict['nmsRQspray'] = terrplant.nmsRQspray(results_dict['EEC-spray-drift'],nms)
        lms = None
        ...
        if results_dict['EEC-total-dry'] and lms:
            results_dict['lmsRQdry'] = terrplant.lmsRQdry(results_dict['EEC-total-dry'],lms)
        if results_dict['EEC-total-semi-aquatic'] and lms:
            results_dict['lmsRQsemi'] = terrplant.lmsRQsemi(results_dict['EEC-total-semi-aquatic'],lms)
            results_dict['lmsRQspray'] = terrplant.lmsRQspray(results_dict['EEC-spray-drift'],lms)
        nds = None
        ...
        if results_dict['EEC-total-dry'] and nds:
            results_dict['ndsRQdry'] = terrplant.ndsRQdry(results_dict['EEC-total-dry'],nds)
        if results_dict['EEC-total-semi-aquatic'] and nds:    
            results_dict['ndsRQsemi'] = terrplant.ndsRQsemi(results_dict['EEC-total-semi-aquatic'],nds)
            results_dict['ndsRQspray'] = terrplant.ndsRQspray(results_dict['EEC-spray-drift'],nds)
        lds = None
        ...
        if results_dict['EEC-total-dry'] and lds:
            results_dict['ldsRQdry'] = terrplant.ldsRQdry(results_dict['EEC-total-dry'],lds)
        if results_dict['EEC-total-semi-aquatic'] and lds:
            results_dict['ldsRQsemi'] = terrplant.ldsRQsemi(results_dict['EEC-total-semi-aquatic'],lds)
            results_dict['ldsRQspray'] = terrplant.ldsRQspray(results_dict['EEC-spray-drift'],lds)            
        return results_dict
