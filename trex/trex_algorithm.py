...
class TRexAlgorithmPage(webapp.RequestHandler):
    def get(self):
        ...
        html = uber_lib.SkinChk(ChkCookie, "TREX Algorithms")
        html = html + template.render(templatepath + '02uberintroblock_wmodellinks.html', {'model':'trex','page':'algorithm'})
        html = html + template.render(templatepath + '03ubertext_links_left.html', {})                      
        html = html + template.render(templatepath + '04uberalgorithm_start.html', {
                'model':'trex', 
                'model_attributes':'TREX Algorithms', 
                'text_paragraph':x})
