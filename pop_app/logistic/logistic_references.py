...
class genericReferencesPage(webapp.RequestHandler):
    def get(self):
        text_file1 = open('logistic/logistic_references.txt','r')
        x = text_file1.read()
        templatepath = os.path.dirname(__file__) + '/../templates/'
        html = template.render(templatepath + '01pop_uberheader.html', {'title':'Ubertool'})
        html = html + template.render(templatepath + '02pop_uberintroblock_wmodellinks.html', {'model':'logistic','page':'references'})
        html = html + template.render(templatepath + '03pop_ubertext_links_left.html', {})                        
        html = html + template.render(templatepath + '04uberreferences_start.html', {
                'model':'terrplant', 
                'model_attributes':'Logistic Model References', 
                'text_paragraph':x})
