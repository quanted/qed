...
def table_all(hedgas_obj):
	...
	html_all = html_all + div_output_start()
	
	if hedgas_obj.run_acuteNonOcc == '1':
	    html_all = html_all + table_7(hedgas_obj)
	if hedgas_obj.run_stitNonOcc == '1':
	    html_all = html_all + table_8(hedgas_obj)
	if hedgas_obj.run_ltNonOcc == '1':
	    html_all = html_all + table_9(hedgas_obj)
	if hedgas_obj.run_acuteOcc == '1':
	    html_all = html_all + table_10(hedgas_obj)
	if hedgas_obj.run_stitOcc == '1':
	    html_all = html_all + table_11(hedgas_obj)
	if hedgas_obj.run_ltOcc == '1':
	    html_all = html_all + table_12(hedgas_obj)
	else:
		html_all = mark_safe("<br><h3>Error: No Model(s) selected</h3>")
	html_all = html_all + div_end()
	return html_all
