...
C
C     + + + DATA INITIALIZATIONS + + +
C     standard aqt/usgs colors (interacter GCOLN)
...
C
C     set the default line patterns for interacter
...
C
C     + + + EXTERNALS + + +
      EXTERNAL   CKWKID, GETMON, IScreenOpen, GCLEAR, SCCLAL, GUNIT
...
C         init interacter graphics and select screen mode
          CALL IScreenOpen (' ','GRB',1280,1024,16)
        ELSE
C         init interacter graphics for hardcopy output
          CALL IScreenOpen (' ','HRW',1280,1024,16)
...
C           output HP-GL file
            CALL GHCSEL (1,1)
C           turn off "replay" info used by interacter toolkit prog.s
            CALL GHCOPT (18,0)
            CALL GDEVIC ('interact.hpg')
          ELSE IF (MONVEC(WKID).EQ.4) THEN
C           output CGM (Computer Graphics Metafile)
            CALL GHCSEL (1,9)
            CALL GDEVIC ('interact.cgm')
          ELSE IF (MONVEC(WKID).EQ.5) THEN
C           output Encapsulated PostScript file
            CALL GHCSEL (1,2)
            CALL GHCOPT (22,1)
            CALL GDEVIC ('interact.eps')
          ELSE IF (MONVEC(WKID).EQ.6) THEN
C           output PostScript file
            CALL GHCSEL (1,2)
            CALL GDEVIC ('interact.ps')
          ELSE IF (MONVEC(WKID).EQ.7) THEN
C           output PCX (PC Paintbrush) file
            CALL GHCSEL (1,6)
            CALL GHCOPT (26,0)
            CALL GDEVIC ('interact.pcx')
          ELSE IF (MONVEC(WKID).EQ.8) THEN
C           output compressed BMP (Windows bitmap) file
            CALL GHCSEL (1,6)
            CALL GHCOPT (26,2)
            CALL GDEVIC ('interact.bmp')
          END IF
        END IF
C       get information for this workstation
     ...
     #              IBKVEC(WKID))
C
C       clear interacter graphics screen
        CALL GCLEAR
C       set interacter area based on graphics device width/height
...
C
      IF (FONT .LE. 1) THEN
        CALL GCHSET ('standard.chr')
      ELSE IF (FONT .EQ. 2) THEN
        CALL GCHSET ('swiss.chr')
      ELSE IF (FONT .EQ. 3) THEN
        CALL GCHSET ('swissbld.chr')
      ELSE IF (FONT .EQ. 4) THEN
        CALL GCHSET ('roman.chr')
      ELSE IF (FONT .EQ. 5) THEN
        CALL GCHSET ('romanbld.chr')
      ELSE IF (FONT .EQ. 6) THEN
        CALL GCHSET ('fixed.chr')
      ELSE IF (FONT .EQ. 7) THEN
        CALL GCHSET ('fixedbld.chr')
      END IF
      IERR= INFOER(1)
...
C
C     + + + EXTERNAL + + +
      EXTERNAL   CKWKID, IScreenClose, GPAGE
C
C     + + + END SPECIFICATIONS + + +
C
      CALL CKWKID(WKID)
C
C     shut down interacter graphics
      CALL GPAGE (' ')
      CALL IScreenClose
C     set the state value for GKS and WORKSTATION
      GKSOPN = 1
...
C
      OKFLAG = YES
C
      IF (FONT.LT.1 .OR. FONT.GT.7) THEN
...
C
      ...
      IF (WFLAG.NE.0) THEN
        WRITE(*,2010)
      END IF
Cmyc  some app.s (e.g., AIDE) write to same error file--don't close it
Cmyc  CLOSE(STDERR)
      GKSOPN = 0
C
      RETURN
...
C     + + + LOCAL VARIABLES + ++
      INTEGER   I
      REAL      XDL, YDL, XDR, YDR, D
C
C     + + + EXTERNALS + + +
      EXTERNAL  GCOLN, GMARK1, GCHSIZ
C
C     + + + END SPECIFICATIONS + + +
C
...
C
      ...
      YDL = PY(1)
C     draw the marker
      CALL GMARK1 (MTYPE,MARKH,XDL,YDL)
      DO 100 I=2,N      
        XDR = PX(I)
        YDR = PY(I)
        IF (MRKOPT.EQ.1) THEN
...
C           draw the marker
            CALL GMARK1 (MTYPE,MARKH,XDR,YDR)
...
C
      ...
      CALL CKCI(CI)
C
C     *** not implemented for interacter
C
      RETURN
      END
C
