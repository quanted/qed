...
def loop_html(file_name, batch_jid):
    with open(file_name, 'rb') as thefile:
        ...
        for kk in range(len(przm_obj_all)):
            ...
            setattr(przm_obj_all[kk], 'x_leachate', [i+j for i,j in zip(przm_obj_all[kk].x_precip, przm_obj_all[kk].x_irr)])

            # batch_header = """
            #     <div class="out_">
            #         <br><H3>Batch Calculation of Iteration %s:</H3>
            #     </div>
            #     """%(przm_obj_all[kk].iter_index0+1)
            # iter_html = iter_html + batch_header + przm_tables.table_all(przm_obj_all[kk])

        save_batch_dic("", [x.__dict__ for x in przm_obj_all], 'przm', 'batch', batch_jid)
        print iter_html
        # return przm_obj_all
...
class przm_class_creator(object):
     def __init__(self, iter, chem_name, NOA, Scenarios, Unit, appdate, apm, apr, cam, depi):
        ...
        self.Ar_l = []
        self.Date_inp = []
        self.CAM_1_l = ['1']*self.NOA
        self.CAM_2_l = ['1']*self.NOA
        self.CAM_3_l = ['1']*self.NOA
        self.CAM_4_l = ['1']*self.NOA
        self.DEPI_l = []

        for i in range(self.NOA):
            self.Apt_l.append('5')
            self.Apt_p.append('Enter your own dates')
            self.Ap_m_l.append(apm[i])
            self.Date_inp.append(appdate[i])
            self.DayRe_l.append('0')
            self.Ar_l.append(float(apr[i]))
            self.DEPI_l.append(depi[i])

            if apm[i] == "1":
                self.CAM_1_l[i]=cam[i]
            elif apm[i] == "2":
                self.CAM_2_l[i]=cam[i]
            elif apm[i] == "3":
                self.CAM_3_l[i]=cam[i]
            elif apm[i] == "4":
                self.CAM_4_l[i]=cam[i]
