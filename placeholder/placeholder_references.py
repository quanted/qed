...
class placeholderReferencesPage(webapp.RequestHandler):
    def get(self):
        text_file1 = open('placeholder/placeholder_references.txt','r')
        x = text_file1.read()
        ...
        html = html + template.render(templatepath + '04uberreferences_start.html', {
                'model':'placeholder', 
                'model_attributes':'Placeholder References', 
                'text_paragraph':x})
...
app = webapp.WSGIApplication([('/.*', placeholderReferencesPage)], debug=True)
