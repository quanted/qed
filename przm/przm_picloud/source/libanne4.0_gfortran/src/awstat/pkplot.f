...
C
C     + + + DUMMY ARGUMENTS + + +
      INTEGER       NPKPLT,NPLOT,HSTFLG, NOCLIM
...
C     + + + LOCAL VARIABLES + + +
     ...
     $             L12, L15, L20, L21, L32, L33, L40, L50, L80, L240,
     $             LENOBS, OLEN, JUST,
...
C     + + + DATA INITIALIZATIONS + + +
      DATA       PP1,     PP0,  EPSILN
     #    /   -2.577,  +2.879,  1.0E-6 /
      DATA L15,L20,L21,L32,L33,L40,L50,L80,L240
     #    / 15, 20, 21, 32, 33, 40, 50, 80, 240 /
     ...
     #   /    0,    0,  ' ', 80*' ', 80*' ', 160*' ' /
      ...
      DATA S7 / 'Confidence limit up ' /
      DATA S8 / 'Confidence limit low' /
      DATA C1 / 'NOTICE - Preliminary computation' /
...
Ckmf  IF (NOCLIM .NE. 1) NCRV = NCRV + 2
      NVAR = 2*NCRV
...
C
C     add date to title
      LOC = STRLNX (L240,TITL) + 1
      TITL(LOC) = '&'
      CALL CVARAR (L21,HEADNG(7)(1:21),L21,TITL(LOC+1))
C
      XTYP = 3
      YTYP(1) = 2
      YTYP(2) = 0
...
Ckmf    DO 70 I = NPLOT1, NPLOT2
Ckmf      IF (CLIML(I) .GT. -9.  .AND.  CLIML(I) .LT. 9) THEN
Ckmf        YX(I) = 10. ** CLIML(I)
Ckmf        IF (YX(I) .GT. PMAX) PMAX = YX(I)
Ckmf        IF (YX(I) .LT. PMIN) PMIN = YX(I)
...
Ckmf    LNTYP(KNTCV) = 3
Ckmf    IDX = IDX + 1
Ckmf    CALL GPDATA ( IDX, LST, YX(NPLOT1), RETCOD )
Ckmf    IDX = IDX + 1
Ckmf    CALL GPDATA ( IDX, LST, FCXPG(NPLOT1), RETCOD )
C       'confidence limits up' plot as dots
Ckmf    KNTCV = KNTCV + 1
Ckmf    CALL CVARAR ( L20, S8, L20, LBC(1,KNTCV) )
Ckmf  write (99,*) '  confidence limits up       yx    fcxpg    climu'
Ckmf    DO 80 I = NPLOT1, NPLOT2
Ckmf      IF (CLIMU(I) .GT. -9.  .AND.  CLIMU(I) .LT. 9) THEN
Ckmf        YX(I) = 10. ** CLIMU(I)
Ckmf        IF (YX(I) .GT. PMAX) PMAX = YX(I)
Ckmf        IF (YX(I) .LT. PMIN) PMIN = YX(I)
...
C     put notice
      FYT = 6.0*SIZEL/YLEN
      FXT = ((32.0+7.0)*SIZEL*0.7)/XLEN
      CPR = 32
      NCHR = 96 
      CALL CVARAR (L32,C1,L32,CTXT(1))
      CALL CVARAR (L32,C2,L32,CTXT(33))
      CALL CVARAR (L32,C3,L32,CTXT(65))
